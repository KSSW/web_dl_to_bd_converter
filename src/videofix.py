...
def main():
    if len(sys.argv) < 3:
        print("Users: python nalu_scanner_insert.py <input.(avc|h264|hevc|h265)> <output>")
        sys.exit(1)
    ...
    print(f"Found {len(nalus)} NAL")
    print("Group by AU and calculate insertion offset...")
    offsets, _aus = group_into_aus(nalus)
    print(f"A total of {len(offsets)} AUs，AUDs will be inserted at these positions. Examples of the first 10 offsets:{offsets[:10]}")
    print("Insert AUD and write the file...")
